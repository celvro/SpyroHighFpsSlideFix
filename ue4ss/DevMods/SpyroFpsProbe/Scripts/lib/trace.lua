...
-- "drift" the drift event id (0 when not drifting); "charge", "turn", "camlock", "sc" and "glide" are
-- the ids of the charge, turn, camera lock, super charge and glide measurements in progress (0 outside them).
...
if file then
    file:write("time,dt,fps_cap,sim_step,air,drift,x,y,z,yaw,vx,vy,vz,input_x,input_y,accel_x,accel_y,move_mode,custom_mode,gravity_scale,floor_walkable,floor_dist,floor_nz,root_motion,pressed_jump,jump_hold_time,jump_max_hold_time,jump_force_remaining,"
        ...
        .. "sc,sc_stage,max_accel,jump_z_velocity,falling_lateral_friction,cam_dist,cam_height,cam_fov,cam_rad_default,glide\n")
end
...
function trace.writeRow(r)
    ...
    file:write(string.format("%d,%s,%.1f,%.1f,%.2f,%.2f,%.2f,%.3f,%.1f,%d\n",
        ...
        state.glide and state.glide.id or 0))
end
