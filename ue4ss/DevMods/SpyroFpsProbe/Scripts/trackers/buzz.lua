-- Buzz (Spyro 3 boss, LS308 Buzz's Dungeon): his ChargeRun state plays the run montage but often
-- doesn't move at high FPS.
--
--   "buzzrun" lines     one per ChargeRun (and RollAttack/RollRetreat) state: duration, distance moved,
--                       average/max speed from position and from Velocity, frames with no displacement,
--                       the state it left to, and the movement settings it ran with.
--   "buzzcmc" dump      every reflected property of his movement component, once per Buzz
--                       (camdump_<time>_buzzcmc.txt).
--   buzz_<stamp>.csv    one row per frame while a Buzz exists.
--
...
local buzz = { CLASSES = { BP_CBS3002_Buzz2_C = true, BP_CBS3002_Buzz2_LS326_C = true } }

local MOVING_STATES = { ChargeRun = true, RollAttack = true, RollRetreat = true }
...
local function newRun(stateName, r, loc, entry)
    return { stateName = stateName, start = r.time, startLoc = loc, frames = 0, dtSum = 0, dist = 0,
             speedMax = 0, velSum = 0, velMax = 0, stillFrames = 0, firstMove = nil,
...
local function logRun(e, run, nextState, cmc, car)
    ...
    log("buzzrun %s state=%s from=%s to=%s cap=%s avgFps=%.1f time=%.3fs dist=%.1f speed=%.1f (max %.1f) velocity=%.1f (max %.1f) stillFrames=%d/%d firstMove=%s maxWalkSpeed=%.1f maxAccel=%.1f car=%s carTurn=%.1f",
    ...
    for _, actor in ipairs(actors) do
        if actor:IsValid() then
            ...
            if not e or r.time < e.start then
                ...
                entries[address] = e
                if cmc:IsValid() then dump.write("buzzcmc", dump.object(cmc)) end
            end
            if cmc:IsValid() then
                ...
                if not file then
                    ...
                    if file then
                        file:write("time,dt,fps_cap,buzz,state,state_time,x,y,z,yaw,disp,speed,vel_x,vel_y,vel_z,vel_speed,accel_x,accel_y,accel_z,move_mode,car,car_turn,rot_rate_yaw,max_walk_speed,max_accel,dilation,spyro_x,spyro_y\n")
                    end
                end
                if file then
                    file:write(string.format("%.5f,%.5f,%s,%s,%s,%s,%.4f,%.4f,%.4f,%.3f,%.5f,%.3f,%.4f,%.4f,%.4f,%.4f,%s,%s,%s,%d,%s,%.2f,%.2f,%.2f,%.1f,%.3f,%.3f,%.3f\n",
